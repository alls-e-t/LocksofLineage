...
```{r Ind-Natal-Coats-Dep-Sexual-Dichromatism-without-Maturation, warning=FALSE}
...
dependent_vars <- c("Sexual_Dichromatism_Complete", "Sexual_Dichromatism_Partial", "Sexual_Dichromatism_Present")
...
```{r Ind-Sexual-Dichromatism-Dep-Natal-Coats-without-Maturation, warning=FALSE}
data <- data_pruned_rownames
phylo_tree <- pruned.tree
independent_vars <- c("Sexual_Dichromatism_Complete", "Sexual_Dichromatism_Partial", "Sexual_Dichromatism_Present")
...
create_heatmap_non_sig_na_grey_global_limits <- function(odds_ratio_matrix, p_value_matrix, title_prefix, global_min, global_max, signif_level = 0.05) {
  # Melt the odds ratio matrix and p-value matrix into data frames
  df_odds <- melt(odds_ratio_matrix)
  df_pvals <- melt(p_value_matrix)
  ...
  # Create the heatmap
  heatmap <- ggplot(df_merged, aes(ColTraits, RowTraits)) + 
    geom_tile(aes(fill = fill), color = "black", size = 0.3) +
    scale_fill_gradient2(low = "#B579A4", high = "#F38C79", mid = "#D8EDDB",
                         midpoint = log(1 + 1), space = "Lab", name="Log Odds Ratio",
                         na.value = "grey", limits = c(global_min, global_max)) +
    geom_text(aes(label = ifelse(is.na(fill), "", sprintf("%.2f", value_log))), size = 3, vjust = 0) +
    geom_text(aes(label = significance), size = 3, vjust = 2) +
    theme_minimal() +
    labs(title = paste(title_prefix, "Log Scale"), x = "Independent Characteristics", y = "Dependent Characteristics") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.text.y = element_text(angle = 45, vjust = 1))
  
  return(heatmap)
}

# Calculating the global limits
all_values <- c()
for (matrix in list(Natal_on_Dichromatism_odds_ratio_matrix, Dichromatism_on_Natal_odds_ratio_matrix, Dimorphism_on_Natal_and_Dichrom_odds_ratio_matrix, Natal_and_Dichrom_on_Dimorphism_odds_ratio_matrix)) { 
    melted_matrix <- melt(matrix)
    all_values <- c(all_values, log(melted_matrix$value + 1))
}

global_min <- min(all_values, na.rm = TRUE)
global_max <- max(all_values, na.rm = TRUE)

# Creating the Heatmaps for the different matrices
heatmapSG1 <- create_heatmap_non_sig_na_grey_global_limits(Natal_on_Dichromatism_odds_ratio_matrix, Natal_on_Dichromatism_p_value_matrix, "Effect of Natal Coat on Sexual Dichromatism", global_min, global_max)
heatmapSG2 <- create_heatmap_non_sig_na_grey_global_limits(Dichromatism_on_Natal_odds_ratio_matrix, Dichromatism_on_Natal_p_value_matrix, "Effect of Sexual Dichromatism on Natal Coat", global_min, global_max)
heatmapSG3 <- create_heatmap_non_sig_na_grey_global_limits( Dimorphism_on_Natal_and_Dichrom_odds_ratio_matrix, Dimorphism_on_Natal_and_Dichrom_p_value_matrix, "Sexual Dimorphism Impacting other Characteristics", global_min, global_max)
heatmapSG4 <- create_heatmap_non_sig_na_grey_global_limits( Natal_and_Dichrom_on_Dimorphism_odds_ratio_matrix, Natal_and_Dichrom_on_Dimorphism_p_value_matrix, "other Characteristics Impacting Sexual Dimorphism", global_min, global_max)
...
upset(
    ...
    base_annotations=list(
        'Intersection size'=intersection_size(
            counts=TRUE,
            mapping=aes(fill=family))  + scale_fill_manual(values = ordered_colors)),
    set_sizes=(
        upset_set_size( mapping = aes(fill=family) +scale_fill_manual(values = ordered_colors),
...
ggvenn(data_pruned_rownames, c("Natal_coat", "Sexual_dichromatism", "Sexual_dimorphism"), fill_color = c("#F38C79","#D8EDDB","#B579A4"))
...
ggvenn(data_pruned_rownames, c("Sexual_dichromatism", "Natal_Coat_Conspicuous", "Natal_Coat_Inconspicuous"), fill_color = c("#F38C79","#D8EDDB","#B579A4"))
...
```{r Subset-Ind-Natal-Coats-Dep-Maturation, warning=FALSE}
data <- nc_data_pruned_rownames
phylo_tree <- pruned.tree
independent_vars <- c("Natal_Coat_Conspicuous", "Natal_Coat_Inconspicuous")
dependent_vars <- c("Maturation_Males_Only", "Maturation_Females_Only", "Maturation_Both")
subset_Natal_on_Maturation_results <- run_phylolm_analyses(data,phylo_tree, independent_vars, dependent_vars)
subset_Natal_on_Maturation_model_stats <- extract_model_stats(subset_Natal_on_Maturation_results, independent_vars, dependent_vars)
subset_Natal_on_Maturation_odds_ratio_matrix <- subset_Natal_on_Maturation_model_stats$odds_ratios
subset_Natal_on_Maturation_p_value_matrix <- subset_Natal_on_Maturation_model_stats$p_values
```

```{r Subset-Ind-Maturation-Dep-Natal-Coats, warning=FALSE}
data <- nc_data_pruned_rownames
phylo_tree <- pruned.tree
independent_vars <- c("Maturation_Males_Only", "Maturation_Females_Only", "Maturation_Both")
dependent_vars <- c("Natal_Coat_Conspicuous", "Natal_Coat_Inconspicuous")
subset_Maturation_on_Natal_results <- run_phylolm_analyses(data,phylo_tree, independent_vars, dependent_vars)
subset_Maturation_on_Natal_model_stats <- extract_model_stats(subset_Maturation_on_Natal_results, independent_vars, dependent_vars)
subset_Maturation_on_Natal_odds_ratio_matrix <- subset_Maturation_on_Natal_model_stats$odds_ratios
subset_Maturation_on_Natal_p_value_matrix <- subset_Maturation_on_Natal_model_stats$p_values
```
...
independent_vars<- c("Sexual_Dichromatism_Complete", "Sexual_Dichromatism_Partial", "Sexual_Dichromatism_Present")
dependent_vars <- c("Maturation_Males_Only", "Maturation_Females_Only", "Maturation_Both")
subset_Sexual_Dichromatism_on_Maturation_results <- run_phylolm_analyses(data,phylo_tree, independent_vars, dependent_vars)
subset_Sexual_Dichromatism_on_Maturation_model_stats <- extract_model_stats(subset_Sexual_Dichromatism_on_Maturation_results, independent_vars, dependent_vars)
subset_Sexual_Dichromatism_on_Maturation_odds_ratio_matrix <- subset_Sexual_Dichromatism_on_Maturation_model_stats$odds_ratios
subset_Sexual_Dichromatism_on_Maturation_p_value_matrix <- subset_Sexual_Dichromatism_on_Maturation_model_stats$p_values

```

```{r Subset-Ind-Maturation-Dep-Sexual-Dichromatism, warning=FALSE}
data <- nc_data_pruned_rownames
phylo_tree <- pruned.tree
independent_vars<- c("Maturation_Males_Only", "Maturation_Females_Only", "Maturation_Both")
dependent_vars <- c("Sexual_Dichromatism_Complete", "Sexual_Dichromatism_Partial", "Sexual_Dichromatism_Present")
subset_Maturation_on_Sexual_Dichromatism_results <- run_phylolm_analyses(data,phylo_tree, independent_vars, dependent_vars)
subset_Maturation_on_Sexual_Dichromatism_model_stats <- extract_model_stats(subset_Maturation_on_Sexual_Dichromatism_results, independent_vars, dependent_vars)
subset_Maturation_on_Sexual_Dichromatism_odds_ratio_matrix <- subset_Maturation_on_Sexual_Dichromatism_model_stats$odds_ratios
subset_Maturation_on_Sexual_Dichromatism_p_value_matrix <- subset_Maturation_on_Sexual_Dichromatism_model_stats$p_values

```

```{r subset-heatmaps, fig.width=16, fig.height=4, dpi=300}
all_values <- c()
for (matrix in list(subset_Natal_on_Maturation_odds_ratio_matrix, subset_Sexual_Dichromatism_on_Maturation_odds_ratio_matrix, subset_Maturation_on_Sexual_Dichromatism_odds_ratio_matrix, subset_Maturation_on_Natal_odds_ratio_matrix)) { 
    melted_matrix <- melt(matrix)
    all_values <- c(all_values, log(melted_matrix$value + 1))
}
...
heatmapSubset1 <- create_heatmap_non_sig_na_grey_global_limits(subset_Natal_on_Maturation_odds_ratio_matrix, subset_Natal_on_Maturation_p_value_matrix, "Subset Effect of Natal Coat on Maturation", global_min, global_max)
heatmapSubset4 <- create_heatmap_non_sig_na_grey_global_limits(subset_Sexual_Dichromatism_on_Maturation_odds_ratio_matrix, subset_Sexual_Dichromatism_on_Maturation_p_value_matrix, "Subset Effect of Sexual Dichromatism on Maturation", global_min, global_max)
heatmapSubset3 <- create_heatmap_non_sig_na_grey_global_limits(subset_Maturation_on_Sexual_Dichromatism_odds_ratio_matrix, subset_Maturation_on_Sexual_Dichromatism_p_value_matrix, "Subset Effect of Maturation on Sexual Dichromatism", global_min, global_max)
heatmapSubset2 <- create_heatmap_non_sig_na_grey_global_limits(subset_Maturation_on_Natal_odds_ratio_matrix, subset_Maturation_on_Natal_p_value_matrix, "Subset Effect of Maturation on Natal Coats", global_min, global_max)

# Print heatmaps side by side
grid.arrange(heatmapSubset1, heatmapSubset2, heatmapSubset3, heatmapSubset4, ncol = 2, nrow = 2)
```
