...
filtered_data <- data_pruned_rownames[is.finite(data_pruned_rownames$DC_Head), ]

# Now generate the sequence using the filtered data
DC_Head_range <- seq(min(filtered_data$DC_Head), max(filtered_data$DC_Head), length.out = 100)
...
new_data <- data.frame(DC_Head = DC_Head_range)
...
new_data$predicted_prob <- 1 / (1 + exp(-(beta_0 + beta_1 * new_data$DC_Head)))

# Step 3: Plotting (assuming new_data is your prediction data frame)
plot(data_pruned_rownames$DC_Head, data_pruned_rownames$DM_Head, col = "red", pch = 16,
     xlab = "DC_Head", ylab = "Probability of DM_Head",
     main = "Observed Data and Predicted Probabilities")
lines(new_data$DC_Head, new_data$predicted_prob, col = "blue", lwd = 2)
